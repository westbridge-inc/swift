...
| UXR-W-002 | W3 | Store/product | hero elevation (flow 3) | [x] | SHIPPED 08-23: MerchantCard treatment at hero scale — name + frosted rating/kind pills ride the photograph, top scrim keeps the chips legible, sheet no longer repeats the name. Device-verified on Android; the photo striping seen while verifying is an emulator artifact, RETRACTED as F-273 |
| UXR-W-015 | W3 | First open (flow 11) | F-266 was scored against a DEAD file (`modules/onboarding/OnboardingScreen.tsx`, zero importers = FG-2 #5). Corrected to the REAL entry — `screens/auth/RolePickerScreen` — and elevated it | [x] | SHIPPED 08-23: brand masthead + 28dp curve carries the reversed mark and the welcome; trio takes per-vertical identity off the F-263 ramp; funnel links become a titled "JUST LOOKING?" card. Device-verified incl. the customer card still routing to Home |
| UXR-W-003 | W3 | Vendor kitchen | first-run/empty state (G3) | [x] | SHIPPED 08-24: the board splits on lifetime orders (`_count.orders`, already on the profile). Zero-ever gets a first-morning card in the house's loud voice with three LIVE rows — real missing-photo count, real open state, real short link — and an honest listening line; everyone else keeps the quiet tile, now carrying `Last order …` so quiet ≠ broken. Device-verified against a real zero-order store |
