...
- **Status (2026-06-20):** RESOLVED BY REMOVAL — `WalletService` and its module/test are gone in V1 (cash-only). The dormant `walletBalance`/`Transaction` schema models remain (reserved for Part C; no code writes to them — the admin order-cancel no longer credits a wallet). The concurrent-debit attack surface no longer exists in V1; the `$transaction` + `Serializable` pattern above is what to reapply when the wallet is actually built (Part C).
