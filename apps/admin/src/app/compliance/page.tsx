    ...
    <div>
      <div className="flex items-center justify-between mb-1">
        ...
        <button
          ...
          className="px-4 py-2 bg-[#E8192C] text-white rounded-lg text-sm font-medium hover:bg-[#E8192C]/80 disabled:opacity-50"
        >
      ...
      <div className="space-y-3 mb-8">
        {violations.length === 0 ? (
          <div className="bg-[#1C1C1E] rounded-xl border border-[#38383A] p-6 text-center text-[#8E8E93]">
            Nobody is operating outside the rules — and the run log below proves it.
          </div>
        ) : (
          violations.map((v) => (
            <div key={v.id} className="bg-[#1C1C1E] rounded-xl border border-red-900/60 p-5">
              <div className="flex flex-wrap items-center gap-3">
                <span className="font-semibold">
                  {v.user?.firstName} {v.user?.lastName}
                </span>
                <span className="text-xs text-[#8E8E93]">{v.user?.phone}</span>
                <span className="text-xs px-2 py-0.5 rounded-full bg-red-500/10 text-red-400 font-bold">
                  {String(v.reason).replaceAll('_', ' ')}
                </span>
                <span className="text-xs text-[#8E8E93]">{v.moverKind}</span>
                <span className="text-xs text-[#8E8E93] ml-auto">{new Date(v.createdAt).toLocaleString()}</span>
              </div>
              ...
              <div className="mt-3 flex gap-2">
                <button
                  ...
                  className="px-3 py-1.5 text-xs rounded-lg border border-[#38383A] hover:bg-[#2C2C2E]"
                >
              ...
              {openEvidence === v.id && (
                <pre className="mt-3 p-3 rounded-lg bg-black/40 text-xs text-[#8E8E93] overflow-x-auto">
          ...
          queue.map((c) => (
            <div key={c.id} className="bg-[#1C1C1E] rounded-xl border border-[#38383A] p-5">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="text-xs text-[#8E8E93]">{c.user?.phone}</span>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                <input
                  ...
                  className="flex-1 min-w-48 bg-[#2C2C2E] px-3 py-1.5 rounded-lg text-sm border border-[#38383A] focus:border-[#E8192C] focus:outline-none"
                />
                <button
      ...
      <div className="bg-[#1C1C1E] rounded-xl border border-[#38383A] overflow-hidden">
        <table className="w-full text-sm">
          <thead className="text-left text-xs text-[#8E8E93] border-b border-[#38383A]">
            <tr>
          ...
          <tbody>
            {runs.length === 0 && (
              <tr>
                <td colSpan={4} className="px-4 py-6 text-center text-[#8E8E93]">
                  No runs yet — the daily job writes one every morning, or run one now.
                </td>
              </tr>
            )}
            {runs.map((r) => (
              <tr key={r.id} className="border-b border-[#38383A]/50 last:border-0">
                <td className="px-4 py-2.5">{new Date(r.startedAt).toLocaleString()}</td>
                <td className="px-4 py-2.5 text-[#8E8E93]">{r.trigger}</td>
