...
type Props = ViewProps & { className?: string; label: string; tone?: 'brand' | 'success' };
...
export function Badge({ className, label, tone = 'brand', ...props }: Props) {
  const bg = tone === 'success' ? 'bg-success/10' : 'bg-brand-50';
  const fg = tone === 'success' ? 'text-success' : 'text-brand-600';
  return (
