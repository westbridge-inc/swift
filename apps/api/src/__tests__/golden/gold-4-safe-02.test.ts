...
const sweep = () => new GuardianService(app.prisma, app.io, { cursorKey: CURSOR_KEY }).sweep(new Date());
...
async function purgeFixtures() {
  await runWithoutTenant(async () => {
    ...
    await app.prisma.sweepCursor.deleteMany({
      where: { workType: { in: [`guardian.open:${CURSOR_KEY}`, `guardian.reconcile:${CURSOR_KEY}`] } },
    });
