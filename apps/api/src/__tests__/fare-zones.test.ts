...
describe('the register’s red test: two tenants, two countries, overlapping polygons, explicit precedence', () => {
  ...
  it('the admin refuses a new zone that overlaps a peer at the same priority; a different priority is accepted; a boundary change is a new version, a copy change is not', async () => {
    ...
    const clash = await app.inject({ method: 'POST', url: '/api/v1/admin/zones', headers, payload: { name: `${TAG} clash`, boundary: CORE_BIGGER, priority: 1 } });
    ...
    const ok = await app.inject({ method: 'POST', url: '/api/v1/admin/zones', headers, payload: { name: `${TAG} above`, boundary: CORE_BIGGER, priority: 3, countryCode: 'gy' } });
    ...
    const reworded = await app.inject({ method: 'PUT', url: `/api/v1/admin/zones/${created.id}`, headers, payload: { description: 'reworded' } });
    expect(reworded.json().data.version).toBe(1);
    const redrawn = await app.inject({ method: 'PUT', url: `/api/v1/admin/zones/${created.id}`, headers, payload: { boundary: box(-58.04, 6.77, -57.97, 6.84) } });
    ...
    const collide = await app.inject({ method: 'PUT', url: `/api/v1/admin/zones/${created.id}`, headers, payload: { priority: 1 } });
