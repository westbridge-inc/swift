 ...
 * written here.
 */
export async function reconcileVendorDerived(prisma: PrismaClient, vendorId: string): Promise<{ added: number; removed: number }> {
  ...
  const removed = stale.length
    ? (await prisma.vendorDiscoveryCategory.deleteMany({
        where: { id: { in: stale.map((s) => s.id) }, source: 'DERIVED' },
      })).count
    : 0;
  return { added, removed };
}

/** Nightly sweep: every vendor with any live item tags. */
export async function reconcileAllDerived(prisma: PrismaClient): Promise<{ vendors: number; added: number; removed: number }> {
  const vendors = await prisma.itemDiscoveryCategory.findMany({
    distinct: ['itemId'],
    select: { itemId: true },
  });
  const items = await prisma.item.findMany({
    where: { id: { in: vendors.map((v) => v.itemId) } },
  ...
  for (const { vendorId } of items) {
    const r = await reconcileVendorDerived(prisma, vendorId);
    added += r.added;
    removed += r.removed;
  }
  return { vendors: items.length, added, removed };
}
