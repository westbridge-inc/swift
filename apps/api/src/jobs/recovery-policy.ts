...
export const JOB_RECOVERY: Record<JobName, Recovery> = {
  ...
  'dispatch-order': { policy: 'SAFE_REPLAY', why: 'The offer cascade is exactly-once per attempt (a live offer short-circuits via offeredIfCurrent; installOfferPair is one atomic Lua reservation refusing ORDER_TAKEN/MOVER_BUSY). The exhaustion tail now carries the BullMQ job id as a one-shot Redis token so a redelivery reuses the committed attempt count instead of re-INCRing it, and its pushes and delayed re-arm carry deterministic dedupeKeys and jobIds that collapse a replay.' },
  ...
  'release-held-orders': { policy: 'SAFE_REPLAY', why: 'The release is a DB compare-and-set: updateMany matches status plus holdExpiresAt <= now and clears holdExpiresAt. A replay\'s findMany filters on holdExpiresAt <= now, which a released (null) row no longer matches, so the second sweep releases nothing, notifies no vendor twice, and arms no duplicate ladder.' },
