...
/**
 * The composition root, importable without booting: server.ts calls
 * buildApp() then listens; tests call buildApp() and inject.
 */
export interface BuildAppOptions {
  /** Receives every route as it is registered (an `onRoute` hook). */
...
export async function buildApp(options: BuildAppOptions = {}) {
  ...
  // [STA-1 deny-readiness] A census of the routes this app serves, for the
