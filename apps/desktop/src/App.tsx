...
export default function App() {
  ...
  const [module, setModule] = useState<'today' | 'review' | 'ops' | 'people' | 'vendors' | 'agent' | 'compliance' | 'health'>('today');
    ...
    <div className="flex min-h-screen">
      <aside className="flex w-52 flex-col border-r border-white/10 p-4">
        ...
        <nav className="mt-5 flex-1 space-y-0.5">
          {([['today', 'Today'], ['review', 'Review'], ['ops', 'Live Ops'], ['people', 'People'], ['vendors', 'Vendors'], ['agent', 'Agent'], ['compliance', 'Compliance'], ['health', 'Health']] as const).map(([key, label]) => (
            <button
              key={key}
      ...
      <main className="min-w-0 flex-1 p-8">
        <h1 className="mb-5 text-2xl font-extrabold">
          {{ today: 'Today', review: 'Review Center', ops: 'Live Ops', people: 'People', vendors: 'Vendors & Billing', agent: 'Agent approvals', compliance: 'Compliance', health: 'Health' }[module]}
        </h1>
        {module === 'today' && <Today />}
